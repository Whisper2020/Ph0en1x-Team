...
void minsn_t::verify(micro_verifier_t &mv, bool with_target) const
{
  ...
  switch ( opcode )
  {
    case m_stx:
    case m_ijmp:
      if ( d.size != 2 && d.size != addrsize(mvm) )
        INTERR(50826); // wrong operand sizes
      if ( r.size != 2 )
        INTERR(50827); // wrong operand size
      break;
    case m_ldx:
      if ( r.size != 2 && r.size != addrsize(mvm) )
        INTERR(50828); // wrong operand size
      if ( l.size != 2 )
        INTERR(50829); // wrong operand size
      break;
    case m_add:
    case m_sub:
    case m_mul:
  ...
  if ( type != BLT_NONE )
  {
    ...
    switch ( type )
    {
      ...
      case BLT_1WAY: // passes execution to one block
        ...
        if ( is_call_block() )
        {
          if ( tail->is_noret_call(hv.mvm) ) // -V595 tail is used before verifying against NULL
