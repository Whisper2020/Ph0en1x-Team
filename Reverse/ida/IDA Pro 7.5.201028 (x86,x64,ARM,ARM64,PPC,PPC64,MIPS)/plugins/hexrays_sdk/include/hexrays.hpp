...
enum merror_t
{
  ...
  MERR_MAX_ERR   = 33,
  MERR_LOOP      = -34, ///< internal code: redo last loop (never reported)
};
//@}
...
public:
  ...
  bool has_user_info(void) const { return (flags & (CVAR_UNAME|CVAR_UTYPE|CVAR_NOPTR)) != 0 || !cmt.empty(); }
...
#define SVW_SOFT  0x02 // may fail and return false;
  ...
  /// \param if true, append padding bytes in case of scattered lvar
  void hexapi append_list(mlist_t *lst, bool pad_if_scattered=false) const;
...
struct ivl_with_name_t
{
  ...
  ivl_with_name_t(): ivl(0, BADADDR), whole("<unnamed inteval>"), part(NULL) {}
  DEFINE_MEMORY_ALLOCATION_FUNCS()
};
...
/// \param bit   microregister number
...
struct op_parent_info_t
{
  ...
  op_parent_info_t(
        mba_t *_mba=NULL,
        mblock_t *_blk=NULL,
        minsn_t *_topins=NULL)
    : mba(_mba), blk(_blk), topins(_topins), curins(NULL) {}
  HEXRAYS_MEMORY_ALLOCATION_FUNCS()
...
struct mlist_mop_visitor_t
{
  ...
  bool changed;
  mlist_t *list;
  mlist_mop_visitor_t(void): topins(NULL), curins(NULL), changed(false), list(NULL) {}
...
struct stkvar_ref_t
{
  ...
  member_t *hexapi get_stkvar(uval_t *p_off=NULL) const;
};
...
struct scif_t : public vdloc_t
{
  ...
  scif_t(mba_t *_mba, qstring *n, tinfo_t *tif) : mba(_mba)
  {
    n->swap(name);
    tif->swap(type);
  }
...
#define OPROP_LOWADDR 0x20 ///< a low address offset
  ...
  const char *hexapi dstr(void) const; // use this function for debugging
  ...
  /// \param m instruction to embed into operand. may not be NULL.
  ...
  /// \param ea address of the nested instruction
  /// Note: this function does not erase the previous contents of the operand;
  ///       call erase() if necessary
  ...
  bool hexapi is_constant(uint64 *out=NULL, bool is_signed=true) const;
...
public:
  ...
  mcallarg_t(void) : ea(BADADDR) {}
  mcallarg_t(const mop_t &rarg) : mop_t(rarg), ea(BADADDR) {}
...
//@}
  ...
  void hexapi append_list(mlist_t *list) const;
...
#define IPROP_MBARRIER  0x80000 ///< this instruction acts as a memory barrier
  ...
  int optimize_solo(int optflags=0) { return optimize_subtree(NULL, NULL, NULL, NULL, optflags); }
...
#define EQ_OPTINSN 0x0008      ///< optimize mop_d operands
  ...
  /// \param ignore_noret_icall if set, indirect calls to noret functions will
  ///                           return false
  ...
  bool contains_call(bool with_helpers=false) const { return find_call(with_helpers) != NULL; }
  ...
  bool hexapi modifes_d(void) const;
  bool modifies_pair_mop(void) const { return d.t == mop_p && modifes_d(); }

  /// Is the instruction in the specified range of instructions?
  /// \param m1 beginning of the range in the doubly linked list
  /// \param m2 end of the range in the doubly linked list (excluded, may be NULL)
...
#define MBL_VALRANGES   0x8000  ///< should optimize using value ranges
  ...
  ///           if NULL, then the instruction will be inserted at the beginning
  ///           of the list
  ...
  ///             i2 is excluded from the range. it can be specified as NULL.
  ...
  /// \param p_i1   ptr to ptr to a top level instruction.
  ///               denotes the beginning of the search range.
  /// \param i2     end instruction of the range (must be a top level insn)
  ///               i2 is excluded from the range. it can be specified as NULL.
  ///               i1 and i2 must belong to the same block.
  /// \fdflags      combination of \ref FD_ bits
...
#define VR_EXACT    0x0002    ///< find exact match. if not set, the returned
  ...
  /// Calculate number of register instructions in the block.
  ...
  bool is_call_block(void) const { return tail != NULL && is_mcode_call(tail->opcode); }
  bool is_unknown_call(void) const { return tail != NULL && tail->is_unknown_call(); }
...
enum warnid_t
{
  ...
  WARN_OPT_VALRNG,    ///< 46 conditional instruction was optimized away because of '%s'
...
#define MBA2_ALL_FLAGS    0x00000FFF
...
//@}
  ...
  /// \param ea       desired address of the operand
  ...
  ea_t hexapi map_fict_ea(ea_t fict_ea);
  ...
  static mba_t *hexapi deserialize(const uchar *bytes, size_t nbytes);
...
class mbl_graph_t : public simple_graph_t
{
  ...
  /// \param m2   ending instruction (in b2). excluded. may be NULL.
...
struct cdg_insn_iterator_t
{
  ...
  bool is_likely_dslot;   // execute delay slot only when jumping
  ...
  cdg_insn_iterator_t(const mba_t *mba_)
    : mba(mba_),
      ...
      dslot(BADADDR),
      severed_branch(BADADDR),
      is_likely_dslot(false) {}
...
public:
  ...
  /// but permits to create virtually any instruction. Operands may be NULL
  /// when it makes sense.
...
//@}
  ...
  bool maybe_ptr(void) const
  {
    ...
    if ( get_const_value(&val)
      && (ea_t(val) != val || !is_mapped((ea_t)val)) )  //-V560 part of conditional expression is always false
    {
      return false;
    }
    return true;
...
void hexapi save_user_labels2(ea_t func_ea, const user_labels_t *user_labels, const cfunc_t *func=NULL);
...
user_labels_t *hexapi restore_user_labels2(ea_t func_ea, const cfunc_t *func=NULL);
...
#define DECOMP_NO_HIDE  0x0020 ///< do not hide display waitbox. see close_hexrays_waitboxes()
#define DECOMP_NO_XREFS 0x0040 ///< do not update global xrefs cache
//@}
...
//@}
  ...
  /// Due to the coordinate system for comments (http://www.hexblog.com/?p=60)
...
///             if NULL or empty then UDTs from the "Local types" will be used
/// \param ops  operands
/// \param cb   callback will be called to apply the selection for every operand
...
inline block_chains_t *block_chains_new(void)
{
...
//--------------------------------------------------------------------------
inline void lvar_t::append_list(mlist_t *lst, bool pad_if_scattered) const
{
  hexdsp(hx_lvar_t_append_list, this, lst, pad_if_scattered);
}
...
{
  return (uchar)(size_t)hexdsp(hx_install_microcode_filter, filter, install) != 0;
}
...
{
  return (uchar)(size_t)hexdsp(hx_remove_optinsn_handler, opt) != 0;
}
...
{
  return (uchar)(size_t)hexdsp(hx_remove_optblock_handler, opt) != 0;
}
...
//--------------------------------------------------------------------------
inline void chain_t::append_list(mlist_t *list) const
{
  hexdsp(hx_chain_t_append_list, this, list);
}
...
//--------------------------------------------------------------------------
inline bool minsn_t::is_noret_call(bool ignore_noret_icall)
{
  return (uchar)(size_t)hexdsp(hx_minsn_t_is_noret_call, this, ignore_noret_icall) != 0;
}
...
//--------------------------------------------------------------------------
inline bool minsn_t::modifes_d(void) const
{
  return (uchar)(size_t)hexdsp(hx_minsn_t_modifes_d, this) != 0;
}
...
//--------------------------------------------------------------------------
inline bool mba_t::remove_empty_blocks(void)
{
  return (uchar)(size_t)hexdsp(hx_mba_t_remove_empty_blocks, this) != 0;
}
...
//--------------------------------------------------------------------------
inline ea_t mba_t::map_fict_ea(ea_t fict_ea)
{
  ea_t retval;
...
//--------------------------------------------------------------------------
inline mba_t *mba_t::deserialize(const uchar *bytes, size_t nbytes)
{
